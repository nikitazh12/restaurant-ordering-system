...
class TestFavoritesEndpoints:
    ...
    def test_get_favorites_authorized(self, client_mock: TestClient, mock_db_session):
    ...
    def test_add_to_favorites_authorized(self, client_mock: TestClient, mock_db_session):
        from backend.auth import get_password_hash
        ...
        headers = {"Authorization": f"Bearer {token}"}
        payload = {"menu_item_id": 1}
        response = client_mock.post("/api/favorites", json=payload, headers=headers)
        assert response.status_code in [201, 400]

    def test_favorites_operations_flow(self, client_mock: TestClient, mock_db_session):
        from backend.auth import get_password_hash
        ...
        mock_db_session.query.return_value.filter.return_value.all.return_value = [fav_item]
